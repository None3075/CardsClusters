...
df_winrate = total_df.join(wins_df, on=["Agent_hand", "Chunk Number", "Distance_from_21"], how="left").fillna(0, subset=["Wins"]).withColumn("Winning Rate Proportion", col("Wins") / col("TotalGames"))
df_winrate.orderBy("Winning Rate Proportion", ascending = False).show(10)
...
df_winrate = total_df.join(wins_df, on=["Agent_hand", "Distance_from_21"], how="left").fillna(0, subset=["Wins"]).withColumn("Winning Rate Proportion", col("Wins") / col("TotalGames"))
df_winrate.orderBy("Winning Rate Proportion", ascending = False).show(1000)